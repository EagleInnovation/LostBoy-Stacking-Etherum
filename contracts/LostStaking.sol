...
contract LostStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    uint public freezingDays;
  
    // mappings 
    mapping(address => EnumerableSet.UintSet) private _deposits;
    mapping(address => mapping(uint256 => uint256)) public _depositBlocks;
    mapping(address => mapping(uint256 => uint256)) public _depositBlocksTmp;
    mapping(address => mapping(uint256 => uint256)) public _depositRarity;
    mapping(uint256 => uint256) public _rewardArray;
    mapping(uint256 => uint256) public _rewardRarity;
    ...
    function setOneDayPrev(address account, uint256 tokenId) public onlyOwner(){
    ...
    function updateReward(uint256[] memory _updateReward) public onlyOwner() {
    ...
    // Set a multiplier for how many tokens to earn each time a block passes.
    function updateRarity(uint256[] memory _updateRarity) public onlyOwner() {
        for (uint256 i; i < _updateRarity.length; i++) {
            _rewardRarity[i] = _updateRarity[i];
        }
    }

    function setfreezingDays(uint _freezDay) public onlyOwner() {
        freezingDays = _freezDay;
    }
    ...
    function withdraw(uint256[] calldata tokenIds) external whenNotPaused nonReentrant() {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            require( diff >= freezingDays , "Freezing nft for Limited days");
